...
public class AlxGifHelper {
    ...
    public static class ProgressViews {
        public ProgressViews(
                WeakReference<GifImageView> gifImageViewWeakReference,
                WeakReference<ImageView> iconImageViewWeakReference,
                WeakReference<ImageView> coverImageViewWeakReference,
        ...
        public WeakReference<ImageView> coverImageViewWeakReference;//coverImage
        public WeakReference<ImageView> iconImageViewWeakReference;//coverImage
    ...
    public static void displayImage(final String url, ImageView gifIcon, GifImageView gifView, ImageView coverView,
        ...
        Log.i("lpftag", "gif图片的缓存路径是" + path);
        final File cacheFile = new File(path);
        if (cacheFile.exists()) {//如果本地已经有了这个gif的缓存
            Log.i("lpftag", "本图片有缓存");
            if (displayImage(cacheFile, gifIcon, gifView, coverView, displayWidth)) {//如果本地缓存读取失败就重新联网下载
                if (progressBar != null) progressBar.setVisibility(View.GONE);
                if (tvProgress != null) tvProgress.setVisibility(View.GONE);
                return;
            }
        }
        //为了防止activity被finish了但是还有很多gif还没有加载完成，
        // 导致activity没有及时被内存回收导致内存泄漏，这里使用弱引用
        final WeakReference<GifImageView> imageViewWait = new WeakReference<GifImageView>(gifView);
        final WeakReference<ImageView> imageViewCover = new WeakReference<ImageView>(coverView);
        final WeakReference<ImageView> imageViewIcon = new WeakReference<ImageView>(gifIcon);
        ...
        //todo
        gifView.setImageResource(R.mipmap.loading);//设置没有下载完成前的默认图片
        if (memoryCache != null && memoryCache.get(url) != null) {//如果以前有别的imageView加载过
            Log.i("lpftag", "以前有别的ImageView申请加载过该gif" + url);
        ...
        ProgressWheel progress = progressBarWait.get();
...
//                Log.i("lpftag","下载gif的进度是"+progress+"%"+"    现在大小"+current+"   总大小"+total);
                ArrayList<ProgressViews> viewses = memoryCache.get(url);
                if (viewses == null) return;
//                Log.i("lpftag","该gif的请求数量是"+viewses.size());
            ...
            public void onSuccess(File file) {
                ...
                for (ProgressViews ws : viewArr) {//遍历所有的进度条和imageView，同时修改所有请求同一个url的进度
                    ...
                    ImageView coverImageView = ws.coverImageViewWeakReference.get();
                    ImageView iconImageView = ws.iconImageViewWeakReference.get();
                    if (gifImageView != null)
                        displayImage(renameFile, iconImageView, gifImageView, coverImageView, ws.displayWidth);
                    //修改进度条
                ...
                Log.i("lpftag", url + "的imageView已经全部加载完毕，共有" + viewArr.size() + "个");
     ...
     */
    public static boolean displayImage(File localFile, ImageView imageViewIcon, GifImageView gifImageView, ImageView coverImage, int displayWidth) {
        if (localFile == null || gifImageView == null) return false;
        Log.i("lpftag", "准备加载gif" + localFile.getAbsolutePath() + "显示宽度为" + displayWidth);
        GifDrawable gifFrom;
        try {
            gifFrom = new GifDrawable(localFile);
            int raw_height = gifFrom.getIntrinsicHeight();
            int raw_width = gifFrom.getIntrinsicWidth();
            Log.i("lpftag", "图片原始height是" + raw_height + "  图片原始宽是:" + raw_width);
...
//                Log.i("lpftag","缩放完的gif是"+imageViewWidth+" X "+imageViewHeight);
...
//                }
//            }else {
//                Log.i("lpftag","按照固定大小进行显示");
//            }
            gifImageView.setImageDrawable(gifFrom);
            if (imageViewIcon.getVisibility() == View.INVISIBLE) {
                gifImageView.setVisibility(View.VISIBLE);
                coverImage.setVisibility(View.INVISIBLE);
            }
            Log.d("lpftag", "显示当前" + localFile.getName());
//            coverImage.setVisibility(View.INVISIBLE);
            return true;
        } catch (IOException e) {
            Log.i("lpftag", "显示gif出现异常", e);
            return false;
        }
    }
